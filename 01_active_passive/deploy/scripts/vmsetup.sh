...
# echo "Deploy ingress"
sudo runuser -l $ADMIN_ACCT -c "helm install --name nginx-ingress stable/nginx-ingress --namespace nginx --set controller.replicaCount=2 --set controller.nodeSelector.\"beta\.kubernetes\.io/os\"=linux --set defaultBackend.nodeSelector.\"beta\.kubernetes\.io/os\"=linux --set controller.service.annotations.\"service\.beta\.kubernetes\.io/azure-dns-label-name\"=\"$AKS_NAME\""
